function [BB,XW,YW,HH,time] = train_LEMON(XTrain_new,YTrain_new,LTrain_new,GTrain_new,BB,HH,param,time)
    tic;
...
%     time_B=[];
    ...
    for i = 1:max_iter
        ...
        % update P
        P_new = ((HH{1,1}+V_new'*V_new)+gamma*eye(nbits)) \ (HH{1,2}+V_new'*LTrain_new);
        
        % update V_new
        Z = B_new*R'+beta*LTrain_new*P_new'...
            +2*alpha*nbits*GTrain_new*(HH{1,3}+GTrain_new'*B_new)...
            -alpha*nbits*ones(n2,1)*(HH{1,4}+ones(1,n2)*B_new);
        ...
        % update R
        [UB, ~, UA] = svd(HH{1,5}+B_new'*V_new);
        R = UA * UB';
        
        % update B
%         tic;
        B_new = sign(2*alpha*nbits*GTrain_new*(HH{1,6}+GTrain_new'*V_new)...
            -alpha*nbits*ones(n2,1)*(HH{1,7}+ones(1,n2)*V_new)+V_new*R);
%         time_B=[time_B toc];
    end
...
%     tic;
    XW = (HH{1,8}+xi*eye(size(XTrain_new,2))) \ HH{1,10};
%     time.Wx=[time.Wx;toc];
%     tic;
    YW = (HH{1,9}+xi*eye(size(YTrain_new,2))) \ HH{1,11};
%     time.Wy=[time.Wy;toc];

%     time.B_new=[time.B_new;time_B];
